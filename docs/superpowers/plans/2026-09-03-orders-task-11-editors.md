### Task 11 — the funnel editors

This is step 1 of [task 11](2026-09-03-orders-task-11-settings-docs.md). It lives in its own
...
Create `rakurs/src/screens/FunnelSettings.tsx`:
...
function StageList({
  ...
  /** Порядок отправляется целиком: сервер принимает только полный список. */
  function move(index: number, delta: number) {
    const next = [...list];
    const [moved] = next.splice(index, 1);
    next.splice(index + delta, 0, moved!);
    return run(() => api.reorderStages(agentId, next.map((stage) => stage.id)));
  }
        ...
        <div key={stage.id} className="sunken-box" style={{ padding: '10px 12px' }}>
          ...
          {open === stage.id && (
            <StageForm
              ...
              onDelete={() =>
                run(() => api.deleteStage(agentId, stage.id), 'Стадия удалена').then(() =>
                  setOpen(null),
                )
              }
            />
...
function StageForm({
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      // Отказ сервера — это «продажа уже есть» или «в стадии есть диалоги». Он написан
      // для человека, и показать его дословно правильнее, чем переписать своими словами.
      toast.fail(error);
    } finally {
      setSaving(false);
    }
...
function FieldList({
  ...
  async function remove(fieldId: string) {
    try {
      await api.deleteLeadField(agentId, fieldId);
      onChanged();
    } catch (error) {
      toast.fail(error);
    }
        ...
        >
          ...
          {!readOnly && (
            <button
              ...
              className="btn-link"
              style={{ marginLeft: 'auto', fontSize: 11.5, color: 'var(--danger)' }}
              // Сказано в кнопке, а не в подтверждении: вместе с полем уходят ответы
              // всех лидов на него, и это не отменить.
