...
export function registerStageRoutes(
  ...
  /** Refuses the change that would leave an agent with no sale stage or with two. */
  async function guardSaleStage(agentId: string, kind: string, exceptId?: string) {
    ...
    const [existing] = await db.select({ name: stages.name }).from(stages).where(where);
    if (existing) {
      throw new ApiError(
        409,
        `Стадия продажи уже есть — «${existing.name}». Сначала поменяйте тип у неё.`,
      );
    }
  }
  ...
  app.post(
    ...
    async (req): Promise<Stage> => {
      ...
      await guardSaleStage(req.agent!.id, parsed.data.kind);
  ...
  app.patch(
    ...
    async (req): Promise<Stage> => {
      ...
      if (parsed.data.kind !== undefined && parsed.data.kind !== current.kind) {
        await guardSaleStage(req.agent!.id, parsed.data.kind, current.id);
