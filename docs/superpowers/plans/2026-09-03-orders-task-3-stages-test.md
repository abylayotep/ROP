...
describe('stages', () => {
  ...
  it('refuses a second sale stage', async () => {
    const res = await app.inject({
      method: 'POST',
      url: `/api/agents/${agentId}/stages`,
      cookies: jar,
    ...
    expect(res.json().message).toContain('Стадия продажи уже есть');
  });
  ...
  it('moves the sale to another stage in one request', async () => {
    ...
    expect((await stageNamed('Продажа')).kind).toBe('failure');
