### Task 4 — the test file

This is step 2 of [task 4](2026-09-03-orders-task-4-lead-api.md). It lives in its own
...
Create `server/test/leads-api.test.ts`:
...
describe('the lead', () => {
  ...
  it("refuses a stage from another company's funnel", async () => {
  ...
  it('refuses an assignee who is not in the account', async () => {
    const res = await app.inject({
      ...
      url: leadUrl(),
      cookies: jar,
      payload: { assignedTo: '00000000-0000-4000-8000-000000000000' },
    });
