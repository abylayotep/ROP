...
export function OrderDialog({
  ...
  async function save(event: FormEvent) {
    ...
    // Запятая — то, как сумму набирают на русской раскладке, а ведущие нули остаются
    // от правки уже набранного. Сервер не примет ни то, ни другое, поэтому приводим
    // здесь, а не отказываем человеку в вводе.
    const normalised = amount.trim().replace(',', '.').replace(/^0+(?=\d)/, '');
