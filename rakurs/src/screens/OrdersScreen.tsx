...
export function OrdersScreen() {
  ...
  return <div className="funnel-screen">
    <div className="funnel-toolbar"><div><h2>Оплаченные заказы</h2><p>Покупки с подтверждённой оплатой.</p></div><Link className="btn" to="../funnel">Открыть воронку</Link></div>
    <Async state={orders} skeleton={<Skeleton height={320} />}>{(data) => {
      ...
      return <>
        ...
        {!data.orders.length ? <EmptyState>Оплаченных заказов пока нет. Заказ появится здесь после оплаты через Kaspi или когда ИИ увидит оплату в переписке.</EmptyState> : !visible.length ? <EmptyState>По этому запросу заказов нет.</EmptyState> : <div className="orders-table-wrap"><table className="orders-table"><thead><tr><th scope="col">Клиент</th><th scope="col">Товар / примечание</th><th scope="col">Сумма</th><th scope="col">Дата оплаты</th><th scope="col">Подтверждение</th></tr></thead><tbody>{visible.map((order) => <tr key={order.id}>
          <td><Link to={`../dialogs?conversation=${encodeURIComponent(order.conversationId)}`}>{formatPhone(order.contactPhone)}</Link></td>
          <td>{order.comment || '—'}</td><td style={{ whiteSpace: 'nowrap', fontWeight: 650 }}>{formatMoney(order.amount, order.currency)}</td><td style={{ whiteSpace: 'nowrap' }}>{order.paidAt ? new Date(order.paidAt).toLocaleString('ru-RU', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '—'}</td><td>{order.operationId ? <><span className="orders-verified">✓ Kaspi · оплачено</span><small>{order.operationId}</small></> : <span className="orders-verified">По переписке</span>}</td>
        </tr>)}</tbody></table></div>}
      </>;
    }}</Async>
  </div>;
