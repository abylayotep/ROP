...
/**
 * What stands where there is no share to state at all — never «0 %».
 *
 * A stage nobody entered has no share of its own, and a step with nothing entered above it
 * has nothing to be a share of. Both are absences; zero would be an accusation.
 */
const NO_VALUE = '—';
    ...
    <Card>
      ...
      <Async state={state} skeleton={<Skeleton height={180} style={{ marginTop: 14 }} />} compactError>
        {(report) => {
          ...
          const overHundred = report.funnel.some(
            (step) => step.conversion !== null && step.conversion > 1,
          );
            ...
            <>
              ...
              {absence === 'off-chain' ? (
                ...
                <div style={{ marginTop: 16 }}>
                  ...
                  {/* Прямо под цепочкой, а не в общем объяснении внизу: владелец, увидев
                      «200 %», решает, что страница сломалась, — и решает это в ту секунду,
                      когда смотрит на число. Показывается только когда такой шаг есть. */}
                  {overHundred && (
                    <div style={hint}>
                      Доля больше 100% — не ошибка: в эту стадию попало больше лидов, чем в
                      ближайшую стадию выше, потому что часть из них перетащили сразу сюда,
              ...
              {!nothingMoved && (
                <div style={hint}>
                  ...
                  только если он в неё действительно входил. Конверсия — доля от ближайшей
                  стадии выше, в которую кто-то заходил; стадии, куда за период никто не
                  заходил, процент не показывают. Цвет полосы — это вид стадии: зелёная —
                  продажа, жёлтая — ожидание оплаты. «Отказ» стоит рядом с цепочкой, а не в
                  ней: отказ — не шаг к продаже.
                </div>
              )}
            </>
          );
 ...
 *
 * Вынесено из разметки, потому что здесь решаются оба неочевидных вопроса карточки.
 *
 * Прочерк вместо процента — это два разных «нечего показывать», и подсказка говорит,
 * какое именно: в саму стадию за период никто не заходил, и доли у неё нет; либо выше по
 * цепочке никто не заходил, и долю считать не от чего. Ноль обвинил бы оператора в потере
 * лидов, которых не было, — поэтому его здесь нет ни в одном случае.
 *
 * Доля больше 100% — не ошибка и не повод её обрезать. Знаменатель — ближайшая стадия
 * выше, в которую кто-то заходил, а лид, которого перетащили сразу сюда, в промежуточные
 * стадии не заходил и в знаменатель не попал.
 */
  ...
  return report.funnel.map((step) => ({
    ...
    pctTitle:
      step.conversion === null
        ? step.entered === 0
          ? 'В эту стадию за период никто не заходил — доли у неё нет'
          : 'Выше по цепочке за период никто не заходил — долю считать не от чего'
        : step.conversion > 1
          ? 'Больше, чем в ближайшей стадии выше: часть лидов перетащили сразу сюда'
          : 'Доля от ближайшей стадии выше, в которую кто-то заходил',
