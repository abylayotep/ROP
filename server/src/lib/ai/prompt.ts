...
export interface PromptStage {
  ...
  description: string;
  /** What the agent should achieve while the lead is on this stage. Empty is normal. */
  goal?: string;
}
...
function roleSection(agent: PromptAgent): string {
  ...
  return [
    `Ты — менеджер по продажам компании «${name ?? 'без названия'}». Ты переписываешься с клиентом в WhatsApp так, как это делает живой внимательный менеджер: сначала понимаешь, что нужно человеку, отвечаешь на его вопросы и помогаешь выбрать, и только потом оформляешь заказ.`,
...
function rulesSection(agent: PromptAgent, guard: string, promotion = false): string {
  ...
  const language =
    chosen === null
      ? 'Отвечай на языке клиента: на каком языке написал он, на таком пиши и ты — весь ответ целиком, от приветствия до последнего слова, без русских вставок в казахский ответ и наоборот. Клиент пишет по-казахски (даже латиницей, с ошибками или вперемешку с русскими словами) — отвечай по-казахски. Язык определяет последнее сообщение клиента, а не язык записей, готовых фраз и твоих прошлых ответов: клиент пишет по-русски — весь ответ по-русски, даже если готовая фраза в записи по-казахски. Записи базы знаний, готовые фразы, шаги скрипта и инструкции могут быть на другом языке: переводи их, а не цитируй.'
...
/**
 * How a sale is talked through, and which step of it this conversation is on.
 *
 * The rules say what the agent may claim; nothing said in what order to talk. Without that the
 * one topic a model saw spelled out — payment — became the topic of the first reply, and a
 * customer's «сәлеметсіз бе» was answered with «will you order?». So the order of a sale is
 * written out, the stage the CRM put the lead on names the current step, and pushing the order
 * before the customer has chosen is named as the mistake it is.
 *
 * The opposite mistake is waiting. «Спроси, что клиенту нужно» read as «Қалай көмектесе
 * аламын?» — a helpdesk line, not a seller's. So the opening is written as a seller opens:
...
function conversationSection(stages: readonly PromptStage[], lead: PromptLead, hasScript = false): string {
  ...
  const goal = current ? quoted(current.goal ?? '').replace(/\n+/g, ' ') : '';
  const step = current === undefined
    ? 'Этапов нет: определи шаг сам по тому, что клиент уже сказал.'
    : [
        `Сейчас сделка на этапе «${inline(current.name, 80)}»${at === -1 ? ' (этап ещё не выбран — считай, что разговор только начался)' : ''}. Этап ставит автоматический разбор, и он может забегать вперёд: если клиент в переписке ещё не сказал, что берёт, действуй по шагам 1–5, даже когда этап дальше.`,
        goal === '' ? 'Цель этапа владелец не написал: определи шаг по порядку продажи выше.' : `Цель на этом этапе: ${goal}`,
        next === undefined ? '' : `Следующий шаг — этап «${inline(next.name, 80)}». Не перескакивай к нему, пока клиент сам туда не пришёл.`,
 ...
 */
const SCRIPT_RECORDS = 'Если в базе знаний есть записи вида script (скрипт продаж компании) — это живые фразы менеджеров этой компании: бери из них манеру и формулировки для своего шага, но факты — всё равно только по ПРАВИЛАМ, а фразу на другом языке, чем пишет клиент, переводи по правилу 3.';
...
/**
 * Checkout, stated with its precondition.
 *
 * It used to sit among the numbered rules, read from the very first message, and the model took
...
function knowledgeSection(items: readonly PromptKnowledge[], guard: string, hasProducts = false): string {
  if (items.length === 0) {
    return [
      ...
      '- приветствие, благодарность, «ок», разговор ни о чём или неясный вопрос: ответь вежливо и коротко и задай конкретный вопрос по шагам ХОД РАЗГОВОРА — не «Чем могу помочь?». handoff не нужен, usedItemIds пустой;',
...
function answerShape(hasScript: boolean): string {
  ...
  '',
  'Пример, когда фактов нет и нужен человек (reply пиши на языке по правилу 3):',
  `{
  ...
  '',
  `И ещё раз главное: факты — только из записей выше, ничего не выдумывать; не хватает сведений — handoff; весь reply на языке последнего сообщения клиента по правилу 3 — готовые фразы на другом языке переводи; ты ведёшь разговор сам — никаких «Чем могу помочь?» / «Қалай көмектесе аламын?», вместо них один конкретный вопрос о товаре или потребности; ${hasScript
