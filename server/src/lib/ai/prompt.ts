...
function rulesSection(agent: PromptAgent, guard: string): string {
  ...
  const language =
    chosen === null
      ? 'Отвечай на языке клиента: на каком языке написал он, на таком пиши и ты — весь ответ целиком, от приветствия до последнего слова, без русских вставок в казахский ответ и наоборот. Клиент пишет по-казахски (даже латиницей, с ошибками или вперемешку с русскими словами) — отвечай по-казахски. Записи базы знаний и инструкции могут быть на другом языке: переводи их, а не цитируй.'
...
function conversationSection(stages: readonly PromptStage[], lead: PromptLead): string {
  ...
  const step = current === undefined
    ...
    : [
        `Сейчас сделка на этапе «${inline(current.name, 80)}»${at === -1 ? ' (этап ещё не выбран — считай, что разговор только начался)' : ''}. Этап ставит автоматический разбор, и он может забегать вперёд: если клиент в переписке ещё не сказал, что берёт, действуй по шагам 1–4, даже когда этап дальше.`,
  ...
  return [
    ...
    '',
    'Главная ошибка — торопить. Пока клиент не выбрал и не сказал, что берёт, не спрашивай «будете заказывать?», «тапсырыс бересіз бе?», не называй реквизиты и не заводи разговор об оплате. Готовые фразы из базы знаний про заказ и оплату используй только на шаге 5.',
    '',
    step,
  ].join('\n');
}
...
const CHECKOUT_SECTION = [
  'ОФОРМЛЕНИЕ И ОПЛАТА. Этот раздел действует только на шаге 5: клиент выбрал товар и сам сказал, что берёт, или сам спросил, как оплатить. До этого его не применяй.',
  ...
  '- Способ оплаты по умолчанию — счёт Kaspi на номер клиента; QR — только если клиент сам попросил. Реквизиты — только из инструкций владельца.',
