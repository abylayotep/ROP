...
import { crmAnalyses, stages, stageTransitions } from '../../db/schema.js';
 ...
 * saw it and no operator has since taken the lead out of the sale stage.
 */
export async function hasVisiblePayment(db: Db, agentId: string, conversationId: string): Promise<boolean> {
  ...
  return seen && !await operatorLeftSale(db, conversationId);
}
