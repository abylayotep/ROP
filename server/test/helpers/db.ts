...
export async function withDb(): Promise<Db> {
  ...
  await db.execute(
    sql`truncate table sessions, account_members, whatsapp_events, messages, notes, lead_values, lead_fields, orders, conversations, stages, contacts, whatsapp_numbers, kb_items, kb_sources, agents, accounts, users restart identity cascade`,
  );
  return db;
}
