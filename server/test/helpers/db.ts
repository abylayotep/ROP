...
export async function withDb(): Promise<Db> {
  ...
  await db.execute(
    sql`truncate table sessions, account_members, whatsapp_events, ai_replies, messages, notes, lead_values, lead_fields, orders, stage_transitions, conversations, stages, contacts, whatsapp_numbers, kb_links, kb_chunks, kb_notes, kb_sources, capi_events, capi_settings, coach_messages, agent_rules, agents, accounts, users restart identity cascade`,
  );
  return db;
}
